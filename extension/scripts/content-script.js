...
var parameterChangeDuration = 0.1;
  ...
  function applySettings() {
    for (var s in settings) {
      ...
      if (s == "enabled") {
        ...
        try {
          target.boost.gain.exponentialRampToValueAtTime(
            value * 4 + 1,
            parameterChangeDuration
          );
        } catch (e) {
        ...
        try {
          ...
          if (s == "threshold") {
            target.compressor[s].linearRampToValueAtTime(
              value,
              parameterChangeDuration
            );
          } else {
            target.compressor[s].exponentialRampToValueAtTime(
              value,
              parameterChangeDuration
            );
          }
